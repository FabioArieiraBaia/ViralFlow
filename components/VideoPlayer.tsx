...
const VideoPlayer = forwardRef<VideoPlayerRef, VideoPlayerProps>(({
  ...
  const renderCanvas = (elapsed: number) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    // FIX: Clear transforms from potential previous aborted transitions
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ctx.globalAlpha = 1.0;
    ctx.filter = 'none';
    ...
    // Ensure transition fits within the scene (max 50% of duration)
    // and effectively starts only at the end
    const MAX_TRANSITION_TIME = sceneDuration / 2;
    const TRANSITION_DURATION = Math.min(1.0, MAX_TRANSITION_TIME);
    
    const timeRemaining = sceneDuration - elapsed;
    const isTransitioning = nextScene && timeRemaining < TRANSITION_DURATION && timeRemaining > 0;
    ...
    if (currentFilter === VideoFilter.SEPIA) filterString = 'sepia(100%) contrast(90%)';
    if (currentFilter === VideoFilter.COLD) filterString = 'hue-rotate(180deg) saturate(50%) brightness(110%)';
    if (currentFilter === VideoFilter.WARM) filterString = 'hue-rotate(-30deg) saturate(120%) contrast(110%)';
    if (currentFilter === VideoFilter.HIGH_CONTRAST) filterString = 'contrast(150%) saturate(110%)';
    ...
    if (!isTransitioning) {
        ...
        let t = 1 - (timeRemaining / TRANSITION_DURATION);
        ...
        if (transitionType === VideoTransition.AUTO) {
             ...
             const types = [VideoTransition.FADE, VideoTransition.SLIDE, VideoTransition.WIPE, VideoTransition.ZOOM, VideoTransition.PIXELATE, VideoTransition.CURTAIN, VideoTransition.ROTATION];
             transitionType = types[currentSceneIndex % types.length];
        }

        // 1. Draw Current Scene (Bottom Layer)
        if (transitionType !== VideoTransition.WIPE && transitionType !== VideoTransition.CURTAIN && transitionType !== VideoTransition.PIXELATE) {
            drawSceneContent(ctx, currentScene, canvas.width, canvas.height, elapsed, sceneDuration, (currentSceneIndex * 137) % 360, s, currentSceneIndex);
        } else {
            drawSceneContent(ctx, currentScene, canvas.width, canvas.height, elapsed, sceneDuration, (currentSceneIndex * 137) % 360, s, currentSceneIndex);
        }
        ...
        if (transitionType === VideoTransition.FADE) {
        ...
        else if (transitionType === VideoTransition.CURTAIN) {
            drawSceneContent(ctx, nextScene, canvas.width, canvas.height, 0, 999, ((currentSceneIndex+1) * 137) % 360, s, currentSceneIndex + 1);
            
            ctx.restore(); 
            ctx.save();
            const gap = (canvas.width / 2) * t;
            ...
            ctx.restore();
        ...
        else if (transitionType === VideoTransition.PIXELATE) {
             // Simple dissolve approximation for pixelate effect without intermediate buffer
             drawSceneContent(ctx, nextScene, canvas.width, canvas.height, 0, 999, ((currentSceneIndex+1) * 137) % 360, s, currentSceneIndex + 1);
             ...
             const blocksToShow = Math.floor(totalBlocks * t);
             
             for(let i=0; i<cols; i++) {
                 for(let j=0; j<rows; j++) {
                      if ( ((i*j + i + j) % totalBlocks) < blocksToShow ) {
                          // Clear/Next Scene
                      } else {
                          ctx.save();
                          ctx.beginPath();
                          ctx.rect(i*blockW, j*blockH, blockW, blockH);
                          ctx.clip();
                          drawSceneContent(ctx, currentScene, canvas.width, canvas.height, elapsed, sceneDuration, (currentSceneIndex * 137) % 360, s, currentSceneIndex);
                          ctx.restore();
                      }
        ...
        else if (transitionType === VideoTransition.ROTATION) {
             ctx.translate(canvas.width/2, canvas.height/2);
        ...
        if (transitionType !== VideoTransition.CURTAIN && transitionType !== VideoTransition.PIXELATE) ctx.restore();
    }
    ...
    if (currentFilter === VideoFilter.NONE || currentFilter === VideoFilter.DREAMY || currentFilter === VideoFilter.HIGH_CONTRAST) {
    ...
    if (showSubs && currentScene.text && currentScene.text.trim().length > 0) {
      ...
      if (subStyle === SubtitleStyle.WORD_BY_WORD) {
           const currentWord = words[currentWordIndex] || "";
           ctx.font = `900 ${fontSize * 2.5}px Inter, sans-serif`;
           ctx.fillStyle = 'black';
           ctx.shadowBlur = 10 * s;
           ctx.shadowColor = 'black';
           ctx.fillText(currentWord, canvas.width/2 + (5*s), canvas.height/2 + (5*s));
    ...
    if (currentScene.overlay) drawOverlay(currentScene.overlay, false);
    if (channelLogoRef.current) drawOverlay(channelLogoRef.current, true);
  ...
  useEffect(() => {
    if (isPlaying && scenes[currentSceneIndex]) {
      ...
      // FIX: Force render the static frame of the current scene immediately
      renderCanvas(0);
    }
    return () => {
      cancelAnimationFrame(rafRef.current);
